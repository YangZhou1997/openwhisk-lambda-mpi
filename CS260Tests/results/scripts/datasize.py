...
def load(filePath):
    x = np.zeros(12)
    latencies = np.zeros((12, 10))
    xcount = 0
    ycount = 0
    for line in open(filePath):
        if line.count('.') == 0:
            dataSize = int(line.rstrip("\n"))
            x[xcount] = dataSize
            xcount += 1
            ycount = 0
        else:
...
x, data_msg = load('../data_trans/msg_data_trans_datasize.txt')
# print data_msg
x, data_redis = load('../data_trans/redis_data_trans_datasize.txt')
# print data_redis
...
print (med_msg - med_redis) / med_redis
...
ax.set_xticks(np.arange(0, 128 * 1024 * 1024 + 1, 16 * 1024 * 1024))

labelX = []
for i in np.arange(0, 128 * 1024 * 1024 + 1, 16 * 1024 * 1024):
    labelX.append(str(2 * i // (1024 * 1024)) + 'MB')
# ax.set_xscale('log')
